...
export default function ScanDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/scans/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(async (res) => {
      if (res.ok) { const d = await res.json(); setScan(d.scan); setIsPublic(!!d.scan.isPublic); if (d.scan.isPublic) setPublicUrl(`${window.location.origin}/report/${d.scan.id}`); }
      setLoading(false);
    });
  }, [id, token]);
  ...
  const toggleShare = async () => {
    ...
    try {
      const next = !isPublic;
      const res = await fetch(`/api/scans/${scan.id}/share`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ public: next }),
      });
      const d = await res.json();
      if (!res.ok) { setShareError(d.error || 'Could not update share status'); return; }
      setIsPublic(next);
      setShareOpen(true);
      setPublicUrl(d.publicUrl || `${window.location.origin}/report/${scan.id}`);
    } catch (e: any) {
      setShareError(e.message || 'Could not update share status');
    }
  };

  const copyLink = async () => {
    try { await navigator.clipboard.writeText(publicUrl); } catch { /* clipboard unavailable */ }
    setCopied(true);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={toggleShare} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 border ${isPublic ? 'bg-emerald-900/30 hover:bg-emerald-900/50 border-emerald-800 text-emerald-400' : 'bg-blue-900/30 hover:bg-blue-900/50 border-blue-800 text-blue-400'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 0l-6.632 3.316m0 0a3 3 0 10-5.367 2.684 3 3 0 005.367-2.684z" /></svg>
      ...
      {/* Share panel */}
      {shareOpen && (
        <div className="bg-blue-950/20 border border-blue-800/60 rounded-xl p-4 mb-8 flex flex-col lg:flex-row lg:items-center gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-sm text-blue-300 font-medium">{isPublic ? 'This report is public. Anyone with the link can view it.' : 'Sharing is disabled for this report.'}</p>
            {isPublic && (
              <div className="flex items-center gap-2 mt-2">
                <input readOnly value={publicUrl} className="flex-1 min-w-0 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs font-mono text-slate-300 outline-none" onFocus={(e) => e.target.select()} />
                <button onClick={copyLink} className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold transition-colors shrink-0">{copied ? 'Copied!' : 'Copy link'}</button>
              </div>
            )}
          ...
          <div className="shrink-0 flex items-center gap-3 text-xs text-slate-400">
            <span>{isPublic ? `Public since ${scan?.sharedAt ? new Date(scan.sharedAt).toLocaleDateString() : 'now'}` : 'Unlisted'}</span>
            <button onClick={toggleShare} className="text-red-400 hover:text-red-300 underline underline-offset-2">{isPublic ? 'Stop sharing' : 'Make public'}</button>
          </div>
        </div>
      )}
