...
export default function PricingPage() {
  ...
  const hasLemon = !!(process.env.NEXT_PUBLIC_LEMONSQUEEZY_ENABLED);
  ...
  const subscribe = async (planId: string) => {
    ...
    if (hasLemon) {
      setLoading(planId);
      const res = await fetch('/api/lemon-squeezy', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ plan: planId }) });
        ...
        <div className="max-w-6xl mx-auto text-center">
            ...
            {plans.map((plan) => (
              <div key={plan.name} className={`rounded-2xl border p-8 text-left ${plan.name === 'Professional' ? 'border-emerald-500 bg-slate-900/80 ring-1 ring-emerald-500/20' : 'border-slate-800 bg-slate-900'}`}>
                ...
                <button onClick={() => subscribe(plan.planId)} disabled={!!loading} className="block text-center w-full py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-semibold disabled:opacity-50 transition-colors">
                  {loading === plan.planId ? 'Loading...' : hasLemon ? 'Subscribe' : 'Get Started'}
                </button>
              </div>
            ))}
          </div>
          ...
          {hasLemon && (
            <div className="mt-12 flex justify-center gap-6 text-slate-500 text-sm">
              <div className="flex items-center gap-2"><span className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center text-lg">💳</span> Visa / Mastercard</div>
              <div className="flex items-center gap-2"><span className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center text-lg">🇺🇸</span> USD</div>
              <div className="flex items-center gap-2"><span className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center text-lg">🔒</span> Secure Checkout</div>
            </div>
          )}
        </div>
      </section>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {formSuccess ? (
              <div className="text-center py-4">
                ...
                <p className="text-slate-400 text-sm mt-2">We will contact you within 24 hours.</p>
