            ...
            {plans.map((plan) => (
              <div key={plan.name} className={`rounded-2xl border p-8 text-left ${plan.name === 'Professional' ? 'border-emerald-500 bg-slate-900/80 ring-1 ring-emerald-500/20' : 'border-slate-800 bg-slate-900'}`}>
                ...
                <div className="space-y-2">
                  ...
                  <button onClick={() => openManual(plan)} className="block text-center w-full py-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-sm transition-colors">
                    or Pay via Bank / Crypto
                  </button>
                </div>
              </div>
            ))}
              ...
              <>
                ...
                <form onSubmit={submitLead} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={2} className="w-full px-4 py-3 rounded-lg bg-slate-950 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-white" placeholder="Company name, preferred payment method..." />
                  </div>
