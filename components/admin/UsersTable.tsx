...
export default function UsersTable({ users, onRefresh }: { users: any[]; onRefresh: () => void }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden">
      <div className="px-8 py-6 border-b border-slate-800"><h3 className="text-xl font-bold">All Users</h3><p className="text-sm text-slate-500 mt-1">{users.length} registered users</p></div>
          ...
          <tbody>
            {users.map((u: any) => (
              <tr key={u.id} className="border-b border-slate-800/60 hover:bg-slate-800/40 transition-colors">
                ...
                <td className="px-8 py-5"><span className={`px-3 py-1 rounded-full text-xs font-bold ${u.role === 'admin' ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>{u.role?.toUpperCase()}</span></td>
                <td className="px-8 py-5"><span className={`px-3 py-1 rounded-full border text-xs font-medium capitalize ${u.org?.plan === 'free' ? 'bg-slate-800 border-slate-700 text-slate-500' : 'bg-slate-800 border-slate-700 text-slate-300'}`}>{u.org?.plan || 'free'}</span></td>
                ...
                <td className="px-8 py-5 text-right">
                  <div className="flex gap-1.5 justify-end items-center">
                    ...
                    {u.role === 'admin' && u.email !== 'egegoker35@gmail.com' && (
                      <button onClick={() => setRole(u.id, 'user')} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-orange-700/50 text-orange-400 hover:bg-orange-600/20 transition-colors">Remove Admin</button>
                    )}
